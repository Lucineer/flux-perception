...
impl Engine {
    ...
    pub fn read(&self) -> FusedSignal {
        let active: Vec<&Sensor> = self.sensors.iter().filter(|s| s.active && s.confidence > self.threshold).collect();
        ...
        let value: f64 = adjusted.iter().zip(weights.iter()).map(|(v, w)| v * w).sum::<f64>() / w_sum;

        let mean = adjusted.iter().zip(weights.iter()).map(|(v, w)| v * w).sum::<f64>() / w_sum;
        let variance = adjusted.iter().zip(weights.iter()).map(|(v, w)| w * (v - mean).powi(2)).sum::<f64>() / w_sum;
    ...
    pub fn agreement(&self) -> f64 {
        ...
        let range = adjusted.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b)) - adjusted.iter().fold(f64::INFINITY, |a, &b| a.min(b));
        if range == 0.0 {
            return 1.0;
        }
...
mod tests {
    ...
    fn test_history_returns_last_n() {
        let mut e = make_engine();
        e.history.push(FusedSignal { value: 1.0, confidence: 0.5, variance: 0.1, source_count: 1, timestamp: 1 });
        e.history.push(FusedSignal { value: 2.0, confidence: 0.6, variance: 0.2, source_count: 2, timestamp: 2 });
        e.history.push(FusedSignal { value: 3.0, confidence: 0.7, variance: 0.3, source_count: 3, timestamp: 3 });
        let h = e.history(2);
        assert_eq!(h.len(), 2);
